(function (Script, Collection) {
  ...
  var getResourceNameFromUrl = function (ctx, url) {
  ...
  var acl = function (ctx, data, config) {
    ...
    var resourceName = getResourceNameFromUrl(ctx, this.req.url)
  ...
  monkeypatch(Collection.prototype, 'find', function (original, ctx, fn) {
    var resourceName = String(getResourceNameFromUrl(ctx, ctx.req.url)).replace(/-/g, '')
